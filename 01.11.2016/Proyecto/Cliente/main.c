...
/**
    ...
    \param [fd] fd
    \param [Window] Ventana en la que se va a mostrar
...
/**
    ...
    \param [Amigos] Vector con las direcciones de la lista
    \return Void
 */
...
/**
    ...
    \param [Window] Ventana en la que se va a mostrar
    \param [Login] Imagen de la ventana
    \param [User] String del User
...
int completar_campos (char* Window, IplImage *Login, char* User, char* Password) // devuelve 1 si completo, 0 si toco esc y 2 si dejo algun campo incompleto
{
	...
	i=0;
	while (aux != 27 && aux != 13) //llenar campo User, usa la misma logica que writemsj
	{
		if (aux != '/') {
			if (aux>31)
			{
	...
	cvShowImage (Window,Login);
	aux= cvWaitKey (0);
	while (aux != 27 && aux != 13) //llenar campo Password y FakePassword, usa la misma logica que writemsj
	{
		if (aux!='/') {
			if (aux>31)
			{
...
/**
    ...
    \param [Img] Imagen a la que se le suma la otra
    \param [txt] Imagen con fondo negro
    \return Void
 */
...
/**
    ...
    \param [Img] Imagen del Chat
    \param [new] Mensaje Nuevo
    \param [Quien] Color de la biñeta
    \return Void
 */
...
void writemsj (int fd, char*Window, IplImage *Img,Ls_Friends*Amigo)  //escanea teclado y al precionar enter lo pone en la "consola" ("window")
{
	...
	while (new != 27) // no esc
	{
		...
		j=0;
		new= (cvWaitKey (100));
		while ((new != 13) && (new != 27)) // ni enter ni esc
		{
			if (new!='/')
			{
...
/**
    ...
    \param [Window] Imagen
    \param [Img] Imagen a ser achicada
    \return Void
 */
